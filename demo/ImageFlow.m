...
-(void) calculatePosition{
    ...
    for (int i=0; i<[images count]; i++) {
        ...
        //记录这张图片的高度
        [imageHeights addObject:[NSNumber numberWithFloat:imageHeight]]; 
        if(i%2==0){//第一列
            NSLog(@"图片%d在第0列, offset是%d, 高度是 %f",i, offset1, imageHeight);
            [imagePos addObject:[NSNumber numberWithInt:offset1 ]];
            offset1 += (imageHeight + 7);
        }else{//第二列
            NSLog(@"图片%d在第1列, offset是%d, 高度是 %f",i, offset2, imageHeight);
...
-(void) pinImage:(UIImage *)image withIndex:(int)index{
    ...
    if(index%2==0){
        ...
        if (index == 7) {
            NSLog(@"view7被放入时的参数: %d,%d,%d,%d",x,y,width,height);
        }
